...
## 2026-09-09の承認（以前のpending記載に優先）

ユーザーが「D08はその案で承認する」と明示回答した。責任不明の切断は**勝者なし（winner=-1）・得点不変で試合中断**するpolicy=`abort-v1`を採用する。再承認は不要。以下の未承認/pendingの説明は採用前の状態・旧記録に対する説明として残す。

04の採用後分岐に従い、共通prefix合意時はRecoveryResolved(abort)と双方保存ACK、到達不能時はlocal abort certificateで旧matchを閉じる。履歴forkはhistoryConflictを維持し、旧pending記録を自動確定しない。責任が明確な通常の片側再起動の敗北ルールは維持する。

承認と実装完了は別。abort-v1のビルド定数・rules hash、保存・通信・UIへの反映とA28の検証は残作業。

別家庭PC2台の試験は、ユーザーが今後協力者を募って実施予定。日程・協力者・環境は未確定で、A36–A38はEXTERNAL_PENDINGを維持する。
...
| D08 | 責任不明は勝者なし・score不変で中断 | **2026-09-09明示承認済み**。abort-v1の実装・受入は残る |
