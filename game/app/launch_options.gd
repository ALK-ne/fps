...
func parse(args: PackedStringArray) -> DuelResult:
	var mapping := {"profile": "profile", "role": "role", "endpoint": "endpoint", "scenario": "scenario", "seed": "seed_value", "instance-lock-port": "instance_lock_port", "test-fault": "test_fault"}
	var i := 0
	while i < args.size():
		var key := args[i].trim_prefix("--")
		if not mapping.has(key) or i + 1 >= args.size(): return DuelResult.failure("INVALID_ARGUMENT")
		if key in ["scenario", "seed", "test-fault"] and not OS.is_debug_build(): return DuelResult.failure("DEBUG_ARGUMENT_REJECTED")
		var value := args[i + 1]
