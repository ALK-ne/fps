...
static func invitation(data: Dictionary) -> String:
	return "AD2:" + Marshalls.raw_to_base64(JSON.stringify(data).to_utf8_buffer()).replace("+", "-").replace("/", "_").trim_suffix("=").trim_suffix("=")

static func parse_invitation(code: String, config: GameConfig) -> DuelResult:
	if code.begins_with("AD1:"): return DuelResult.failure("VERSION_MISMATCH")
	if code.length() > 2048 or not code.begins_with("AD2:"): return DuelResult.failure("INVALID_INVITATION")
	...
	if data.v != 2: return DuelResult.failure("VERSION_MISMATCH")
	if not data.host is String or data.host.length() > 253 or data.host.is_empty() or not (data.port is int or data.port is float) or not is_finite(float(data.port)) or data.port != int(data.port) or int(data.port) < 1024 or int(data.port) > 65535: return DuelResult.failure("INVALID_INVITATION")
...
static func session_key(secret: PackedByteArray, match_id: PackedByteArray, epoch: int, host_nonce: PackedByteArray, guest_nonce: PackedByteArray, host_boot: PackedByteArray, guest_boot: PackedByteArray) -> PackedByteArray:
	...
	s.put_data("AD2-session".to_ascii_buffer())
	s.put_data(match_id)
	s.put_u32(epoch)
