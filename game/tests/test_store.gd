...
func test_append_replay(a: DuelAssertions) -> void:
	...
	var created := MatchEvent.make(1, {"match_id": mid, "rule_hash": DuelIds.random_bytes(32), "players": [{"slot": 0}, {"slot": 1}], "epoch": 1})
	a.truth(store.append_transaction([created, MatchEvent.make(2, {"round": 1}), MatchEvent.make(3, {"round": 1}), MatchEvent.make(4, {"round": 1, "winner": 0})]).ok, "durable chain")
